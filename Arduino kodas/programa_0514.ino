...
// duomenys skirti dirbti su duomenu baze
const char* ssid  = "justo_redmi";
const char* password = "12345678";
const char* host = "DESKTOP-V5VPSVB";
const char* passcode = "band";
...
void arNuskaityta()
{
  if (skaitytuvas.available())
  {
    //paimam is RTC laika
    RtcDateTime dabar = Laikrodis.GetDateTime();
    int temp_metai = dabar.Year();
    int8_t temp_menesis = dabar.Month();
    int8_t temp_diena = dabar.Day();
    int8_t temp_valandos = dabar.Hour();
    int8_t temp_minutes = dabar.Minute();
    int8_t temp_sekundes = dabar.Second();
    ...
    if (indeksas != -1)
    {
      ...
      // irasyti i atvaidavima, sd ir db

      // rasymas i atvaizdavima
      darb_vaizdavimas tarp;                          //tarpinis kint
      memcpy(tarp.vardPav, A[indeksas].vardPav, 30);
      sprintf(tarp.data, "%d/%d/%d %d%d:%d%d", temp_metai % 1000, temp_menesis, temp_diena, temp_valandos < 10 ? 0 : temp_valandos / 10, temp_valandos % 10, temp_minutes < 10 ? 0 : temp_minutes / 10, temp_minutes % 10 );
      Darb_iterpimas(tarp);

      // rasymas i duomenu baze

      client.connect(host, httpPort);
      String url = "/projektas/index.php?varpav=";
      url += "TestasIsProjektinio";
      url += "&data=";
      url += temp_metai;
      url += "/";
      url += temp_menesis;
    ...
    else  //jeigu nezinomas zmogus dar
    {
      if (esamas_puslapis != 5)
      {
        buves_puslapis = esamas_puslapis;
      ...
      pranesimas.setText("Neatpazintas asmuo");
      buves_laikas1 = millis();

      // irasyti i atvaidavima, sd ir db
      // rasymas i atvaizdavima
      darb_vaizdavimas tarp;
      memcpy(tarp.vardPav, "Nezinomas asmuo", 30);
      sprintf(tarp.data, "%d/%d/%d %d%d:%d%d", temp_metai % 1000, temp_menesis, temp_diena, temp_valandos < 10 ? 0 : temp_valandos / 10, temp_valandos % 10, temp_minutes < 10 ? 0 : temp_minutes / 10, temp_minutes % 10 );
      Darb_iterpimas(tarp);
